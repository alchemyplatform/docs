...
Notice that the configuration above is in `mainnet-beta`. In order for us to do some testing, we must change this to a devnet URL. We'll be using a dedicated devnet RPC from [Alchemy](https://dashboard.alchemy.com/signup). If you don't already have an account, click on [this link](https://dashboard.alchemy.com/signup) and sign up to create an API key.
