...
Now that we’re inside our project folder, we’ll use `npm init --yes` to initialize the project. If you don’t already have npm installed, follow [these instructions](/docs/alchemy/guides/alchemy-for-macs#1-install-nodejs-and-npm) (we’ll also need **NodeJS** so download that too!).
...
1. [eth\_sendRawTransaction](/docs/alchemy/apis/ethereum/eth_sendrawtransaction), which is the request to actually write our contract onto the Polygon chain, and

2. [eth\_getTransactionByHash](/docs/alchemy/apis/ethereum/eth_gettransactionbyhash) which is a request to read information about our transaction given the hash (a typical pattern when sending transactions).
