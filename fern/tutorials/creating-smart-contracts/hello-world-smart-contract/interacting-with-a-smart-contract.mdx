...
While you are running that script, you may notice that the **Updating the message...** step takes a while to load before the new message is sent. That is due to the mining process! If you are curious about how to track transactions while they are being mined, visit the [Alchemy mempool](https://dashboard.alchemy.com/signup) to see the status of your transaction (whether it's pending, mined, or got dropped by the network). If your transaction got dropped, it's also helpful to check [Sepolia Etherscan](https://sepolia.etherscan.io/) and search for your transaction hash.
