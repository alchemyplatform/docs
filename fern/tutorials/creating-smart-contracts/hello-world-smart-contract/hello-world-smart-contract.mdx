...
subtitle: This is a step-by-step introductory tutorial that will teach you how to create and deploy a smart contract on Ethereum.
...
Now that we’re inside our project folder, we’ll use `npm init` to initialize the project. If you don’t already have npm installed, follow [these instructions](/docs/alchemy/guides/alchemy-for-macs#1-install-nodejs-and-npm) (we’ll also need Node.js so download that too!).
...
You might get a warning about `SPDX license identifier not provided in source file`, but no need to worry about that — hopefully, everything else looks good! If not, please contact us at <Markdown src="../../../../snippets/support-email.mdx" /> or open a ticket in the dashboard.
