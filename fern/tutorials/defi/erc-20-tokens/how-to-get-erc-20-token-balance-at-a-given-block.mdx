...
subtitle: Learn how to get the balance of a single token being held by a specific contract address at a given block or point in time.
...
<CodeGroup>
  ...
  const main = async () => {
      ...
      // Get latest USDT balance using alchemy_getTokenBalances
      const requestBody = {
          "jsonrpc": "2.0",
      ...
      try {
          ...
          let balance = data.result.tokenBalances[0].tokenBalance;
...
<CodeGroup>
  ...
  const main = async () => {
      ...
      const formattedBalance = (parseInt(balance) / 10 ** numDecimals).toFixed(2);
      console.log("Balance:", formattedBalance, "USDT");
  };
