...
description: Learn how to get balances of all tokens owned by an address using the Alchemy Token API.
subtitle: Learn how to get balances of all tokens owned by an address using the Alchemy Token API.
url: https://docs.alchemy.com/docs/how-to-get-all-tokens-owned-by-an-address
...
We will write a simple script in Node to get the balances of the top 100 tokens (by volume) on the Ethereum blockchain using [a free Alchemy developer account](https://dashboard.alchemy.com/signup) and Alchemy's Token API.
...
![880](6fad4ae-Alc_signUp.png "Alc_signUp.png")
...
To make requests to the Token API, use direct HTTP requests with fetch or axios.
...
<CodeGroup>
  ...
  async function main() {
    ...
    for (let token of nonZeroBalances) {
      ...
      let res2 = await fetch(url, loopOptions);
