...
subtitle: Learn how to get the ETH balance of a wallet at a specific block number or UTC timestamp
...
We will write a simple script in Node that gets us the ETH balance of a wallet (say, `vitalik.eth`) at the latest block and at a particular UTC timestamp using [a free Alchemy developer account](https://dashboard.alchemy.com/signup) and the [Ethereum API](/reference/ethereum-api-quickstart).
...
<CodeGroup>
  ...
  // Create EthDater instance with a custom provider function
  const customProvider = {
    getBlock: (blockTag) => client.getBlock({ blockTag }),
    getBlockNumber: () => client.getBlockNumber()
  }
  const dater = new EthDater(customProvider)
