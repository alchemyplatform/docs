...
It makes sense that permanent data, like mined transactions, and ephemeral data, like Ethereum accounts (balance, nonce, etc), should be stored _separately_. Merkle trees, again, are perfect for permanent data. PMTs are perfect for ephemeral data, which Ethereum is in plenty supply of.
...
**The transaction trie records transactions in Ethereum**. Once the block is mined, the transaction trie is _never_ updated.
