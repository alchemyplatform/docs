...
Going back to our lessons on Ethereum Nodes and the low-level [JSON-RPC endpoints](/docs/how-to-read-data-with-json-rpc), we could also use `eth_getLogs` to get at the same log data. The logs in the screenshot below were reading using [Alchemy's Composer tool](https://dashboard.alchemy.com/signup)
