...
subtitle: Learn how to integrate Alchemy’s address activity webhook into Zapier to get email notifications for external transfers when you send and receive ETH via Metamask on the Goerli testnet.
...
In this tutorial, we will be setting up Alchemy’s **address activity webhook** (one of the four types of [Alchemy Webhooks](/docs/alchemy/enhanced-apis/notify-api/using-notify)), which tracks all ETH and token transfer events for an Ethereum address.
...
You can also [choose another testnet](/docs/alchemy/guides/choosing-a-network) for this tutorial, but we recommend using Goerli as all other testnets on Ethereum are expected to be deprecated after [the Merge](https://www.alchemy.com/overviews/the-ethereum-merge).
...
1. [An Alchemy account](https://dashboard.alchemyapi.io/signup/?a=c3e69d5930) (and [Notify API ](/docs/alchemy/enhanced-apis/notify-api#network)from Alchemy’s Enhanced API Product, but not required)
...
And if you do, don’t forget to share it with us on Twitter. If you have any questions or feedback, please contact us at <Markdown src="../../../snippets/support-email.mdx" /> or open a ticket in the dashboard.
