...
Now, let's create an empty repository and install all node dependencies. To make requests to the NFT API, we recommend using the [Alchemy SDK](/docs/alchemy/sdk/sdk-quickstart). However, you can also use `axios` or `fetch` libraries. Run the following commands in your terminal:
