...
subtitle: Learn how to get the \day 1\ minters of an NFT collection. The addresses that originally minted an NFT collection (first owners).
    ...
    while (firstCall || pageKey) {
      let requestBody = {
        ...
        params: [{
          ...
          ...(pageKey && { pageKey: pageKey }) // Add pageKey if it exists
        }]
      };
...
* We first import the necessary Web3 libraries to be able to use them.
* Then we configure the connection with the API key and the correct network.
...
<CodeGroup>
  ...
  const apiKey = "demo";
  const baseURL = `https://eth-mainnet.g.alchemy.com/v2/${apiKey}`;
  const nftBaseURL = `https://eth-mainnet.g.alchemy.com/nft/v2/${apiKey}`;
  ...
  async function checkIfHolder() {
    ...
    const ownersUrl = `${nftBaseURL}/getOwnersForCollection?contractAddress=${nftAddress}`;
    ...
    if (ownersData.error) {
      console.error('Error getting owners:', ownersData.error);
      return;
    }

    let currentOwners = ownersData.ownerAddresses; // Getting the `ownerAddresses` array from the response.
