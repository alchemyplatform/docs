...
description: Learn how to get the last sale price of an NFT using modern Web3 libraries
subtitle: Learn how to get the last sale price of an NFT using modern Web3 libraries
...
The gameplan is simple, we will use the [`getNFTSales`](/reference/sdk-getnftsales) endpoint to get the sales information about the NFT, in addition to this we will sort the response in descending order (most recent sales first), this implies that the first sales object will be the information about the most recent sales.
...
<CodeGroup>
  ...
  async function main() {
    ...
    const url = `${baseURL}/getNFTSales?contractAddress=${contractAddress}&tokenId=${tokenId}&fromBlock=0&toBlock=latest&order=desc`;
    ...
    try {
      ...
      if (!sales.nftSales || sales.nftSales.length === 0) {
        console.log('No sales found for this NFT');
        return;
      }
...
* Configures the connection by setting a network and API key.
...
Now you know how to programmatically get the last sale price of an NFT using modern Web3 libraries!
