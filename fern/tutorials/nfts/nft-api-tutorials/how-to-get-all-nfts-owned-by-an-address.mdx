...
subtitle: Learn how to get all NFTs (and their metadata) owned by an address using the Alchemy NFT API.
...
This will be achieved using a [free Alchemy developer account](https://dashboard.alchemy.com/signup) and the NFT API.
...
<CodeGroup>
  ...
  const main = async () => {
    ...
    const owner = "elanhapern.eth";

    // Get all NFTs
    const response = await fetch(`${baseURL}/getNFTs/?owner=${owner}`, {
      method: 'GET',
    });
...
<CodeGroup>
  ...
  const main = async () => {
    ...
    // Get all NFTs
    const response = await fetch(`${baseURL}/getNFTs/?owner=${address}`, {
      method: 'GET',
    });
