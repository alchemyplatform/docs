...
subtitle: Learn how to identify and filter spam NFTs using the Alchemy API.
slug: docs/how-to-filter-out-spam-nfts
---

<Info>
  This tutorial uses Alchemy's NFT API to identify and filter spam NFTs using direct HTTP requests.
</Info>
...
Fortunately, you can bypass much of the above effort by using [Alchemy’s NFT API](/docs/alchemy/enhanced-apis/nft-api). Alchemy has already completed much of the heavy lifting of identifying spam contracts and makes that data available through easy-to-use API endpoints.
...
<Info>
  If you are new to using the NFT API, check out the [NFT API Quickstart Guide](/docs/alchemy/enhanced-apis/nft-api/nft-api-quickstart-guide) to learn more about how the NFT API works.
</Info>
...
To make requests to the NFT API, you can use `fetch` or `axios`. From your terminal, run the following commands:
...
Now, let's retrieve a list of all NFTs owned by a particular wallet (if you're not sure how to do this, check out [this tutorial](/docs/alchemy/enhanced-apis/nft-api/how-to-get-all-nfts-owned-by-an-address)).
