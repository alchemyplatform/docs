...
subtitle: Learn how to get snapshot of all the owners of an NFT collection at a particular point in time or block height
...
We will write a simple script in Node to gets us all the owners of the *Bored Ape Yacht Club* collection at block number 15753215 using [a free Alchemy developer account](https://dashboard.alchemy.com/signup) and the [NFT API](/reference/nft-api-quickstart).
...
<CodeGroup>
  ...
  const url = `${baseURL}?contractAddress=${address}&block=${block}`;
