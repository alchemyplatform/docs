...
subtitle: Learn how to get a list of all wallets that own a specific NFT collection.
...
We will write a simple script in Node to gets us all the owners of the *World of Women* collection using [a free Alchemy developer account](https://dashboard.alchemy.com/signup) and the [NFT API](/reference/nft-api-quickstart).
...
<CodeGroup>
  ...
  const baseURL = `https://eth-mainnet.g.alchemy.com/nft/v2/${apiKey}/getOwnersForCollection`;
  const url = `${baseURL}?contractAddress=${address}`;
...
If you enjoyed this tutorial on how to get all holders of an NFT collection, give us a tweet [@Alchemy](https://twitter.com/Alchemy).
