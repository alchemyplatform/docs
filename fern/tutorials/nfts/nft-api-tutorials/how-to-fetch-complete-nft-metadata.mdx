...
1. [Sign up for a free Alchemy account](https://dashboard.alchemy.com/signup).
...
1. Run the following commands in order to create your node project.
...
The script below utilizes [getNFTMetadata](/reference/getnftmetadata) API through direct HTTP requests to fetch the metadata for of [World of Women](https://opensea.io/collection/world-of-women-nft) NFT Collection:
...
<CodeGroup>
  ...
  async function getNFTMetadata(nftContractAddress, tokenId) {
    try {
      ...
      const url = `${baseURL}/getNFTMetadata?contractAddress=${nftContractAddress}&tokenId=${tokenId}`;
      ...
      return data; // returning the metadata
    } catch (error) {
...
* The script first defines the required constants including API key and base URL
* It configures the HTTP request parameters to make calls to the Alchemy API
