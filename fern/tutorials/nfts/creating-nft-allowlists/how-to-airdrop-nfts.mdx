...
description: Learn how to airdrop NFTs using both on-chain and off-chain implementations
subtitle: Learn how to airdrop NFTs using both on-chain and off-chain implementations
url: https://docs.alchemy.com/docs/how-to-airdrop-nfts
...
![750](dcb51bb-Untitled.png "Untitled.png")
...
Getting a list of all wallets that hold an NFT of a particular collection is extremely simple using Alchemy's NFT API. [Sign up for a free Alchemy account](https://dashboard.alchemy.com/signup), and use the following code snippet to get a list of all wallets that have a BAYC NFT.
...
<CodeGroup>
  ...
  const main = async () => {
      ...
      // Get owners using Alchemy NFT API
      const url = `${baseURL}/getOwnersForContract?contractAddress=${address}&block=${block}`;
...
To learn more about creating an Alchemy App, and accessing the NFT API using modern Web3 libraries, check out our article on [How to get NFT Owners at a Specific Block Height](/docs/how-to-get-nft-owners-at-a-specific-block-height).
