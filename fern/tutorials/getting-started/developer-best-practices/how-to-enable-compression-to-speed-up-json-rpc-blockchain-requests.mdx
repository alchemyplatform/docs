...
Because of this, the [best practice for making calls to RPC node providers](/docs/alchemy/documentation/best-practices-when-using-alchemy) includes:
...
To compare the latency improvements you’ll get using gzip on a single request, we’ll set up a script to record the total time a [`trace_replayTransaction`](/docs/alchemy/enhanced-apis/trace-api/trace_replaytransaction) request takes, and then run the same request with gzip compression and without gzip compression
