...
In this section, we will use Alchemy's [Transfer API](/reference/transfers-api) to retrieve the contract's first transfer event. We can call the [`alchemy_getAssetTransfers`](/reference/alchemy-getassettransfers) function and filter transfers by passing in the following object parameters:
