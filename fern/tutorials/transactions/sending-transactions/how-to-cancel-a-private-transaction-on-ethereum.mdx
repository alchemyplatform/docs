...
Once your transaction gets confirmed on the Ethereum network, you cannot cancel it. However, you can effectively cancel transactions still in the [Mempool](/docs/alchemy/guides/ethereum-transactions-pending-mined-dropped-and-replaced#what-is-a-mempool) by creating a second transaction with the same [nonce](/docs/alchemy/guides/ethereum-transactions-pending-mined-dropped-and-replaced#what-is-a-nonce) (number only used once) but a higher gas fee. Since miners are incentivized to include the second transaction (with the higher gas fee) first, the nonce will be "used," and the original transaction will be dropped from the mempool.
...
* NodeJS and npm installed ([Install NodeJs and NPM](/docs/alchemy/guides/alchemy-for-macs#1-install-nodejs-and-npm)).
...
* [Alchemy's SDK](/docs/alchemy/sdk/sdk-quickstart#alchemy-sdk-for-javascript)
...
* `maxPriorityFeePerGas`: The miner's tip. (Check out [Alchemy's guide](/docs/alchemy/guides/eip-1559/maxpriorityfeepergas-vs-maxfeepergas#what-is-max-priority-fee-per-gas) to learn more)
