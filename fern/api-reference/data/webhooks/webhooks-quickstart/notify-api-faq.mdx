...
We support 80+ chains. Check [here](https://dashboard.alchemy.com/signup) for the most updated view of which networks are supported for webhooks (or other Alchemy products).
...
We have a number of resources to help you accomplish this! The templates in the [webhooks creation flow](https://dashboard.alchemy.com/signup) cover most common use cases, and there are some more examples in we have shared in [this public repo](https://github.com/alchemyplatform/lab/tree/main/webhooks/queries).
...
* For Custom webhooks, it’s possible your query was configured to fire on all blocks, regardless if the filter matches. When creating a custom webhook from the dashboard, ensure the `Include Empty Messages` checkbox is unchecked. If creating a custom webhook via API, be sure to set the `skip_empty_messages` parameter to `true`.
...
Your webhooks can be automatically paused if you hit your account’s capacity limit or your webhook listener consistently responds with non-2xx HTTP status codes. Go to your [webhook dashboard](https://dashboard.alchemy.com/signup) and hover your cursor over the status to understand the exact reason!
...
The capacity limit error means you’ve exceeded your total monthly compute units. To upgrade your capacity limits, go to the [billing page on your dashboard](https://dashboard.alchemy.com/signup). Below is an example capacity limit error.
...
<CodeGroup>
  ```shell Capacity limit error
  {
    ...
    "event": {
      "error": "Monthly capacity limit exceeded. Visit https://dashboard.alchemy.com/signup to upgrade your scaling policy for continued service."
    }
  }
  ```
</CodeGroup>
...
Ensure your filters and webhooks listener are properly configured! If you have any questions or feedback, please contact us at <Markdown src="../../../../../snippets/support-email.mdx" /> or open a ticket in the dashboard.
...
To find your Auth Token to use the webhook API, navigate to the [webhooks dashboard](https://dashboard.alchemy.com/signup), and copy the Auth Token from the top right of the page.

### Where can I find my webhook's Signing Key?

To find your signing key, navigate to the [webhooks dashboard](https://dashboard.alchemy.com/signup), select your webhook, and copy the singing key from the top right of that webhook’s detail page.
