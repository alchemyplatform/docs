...
Lastly, you'll also need an Alchemy API key and a gas [manager policy ID.](https://www.alchemy.com/docs/wallets/react/sponsor-gas)
...
const preparedUO = await client.prepareCalls({
  ...
  // "capabilities" is a data structure that hold paymaster data (as seen below) or permission data
  capabilities: {
    paymasterService: {
      policyId: TEST_PAYMASTER_POLICY_ID, // put your paymaster policy ID here
    },
  },
});
...
See [here for a list of permissions!](#permission-types)
...
Note that the expiry is in seconds and represents a UNIX timestamp.
