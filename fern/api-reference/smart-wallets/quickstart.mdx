...
In the “context” field under “permissions” you need to pass the concatenated hex of `0x00` (the context version indicator), the  session ID, and the signature. If you are not using session keys, you can omit the "permissions" parameter from the capabilities object.
