...
subtitle: Learn to nteract with Wallet APIs using any RPC client!
...
The logical flow is to get an account for a given signer, create a session for use with that account, prepare the calls you’re looking to send, and send them!
