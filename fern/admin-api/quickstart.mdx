...
curl -X POST "$ALCHEMY_ADMIN_BASE_URL/apps/$APP_ID/networks" \
  -H "Content-Type: application/json" \
  -H "$ALCHEMY_ADMIN_AUTH_HEADER" \
  -d '{
...
curl -X POST "$ALCHEMY_ADMIN_BASE_URL/apps/$APP_ID/origin-allowlist" \
  -H "Content-Type: application/json" \
  -H "$ALCHEMY_ADMIN_AUTH_HEADER" \
  -d '{
    "originAllowlist": [
...
curl -X POST "$ALCHEMY_ADMIN_BASE_URL/apps/$APP_ID/ip-allowlist" \
  -H "Content-Type: application/json" \
  -H "$ALCHEMY_ADMIN_AUTH_HEADER" \
  -d '{
    "ipAllowlist": [
