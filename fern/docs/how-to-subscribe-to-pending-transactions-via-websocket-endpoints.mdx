...
subtitle: >-
  Learn how to subscribe to mined transactions via WebSockets, and view the full
  transactions objects or hashes mined on the network based on specified filters
  and block tags.
...
<Warning title = "Notice stream of results">
...
<Warning title = "Notice new filters">
  ...
  - addresses is an array with two objects that specify the mined transactions based on the address `from` and `to`.
  - includeRemoved set to `true` includes transactions that have been removed from the chain.
  - hashesOnly set to `false` returns a full transaction object.
