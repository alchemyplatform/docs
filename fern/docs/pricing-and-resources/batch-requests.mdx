...
- [Transfers API](ref:transfers-api-quickstart)
- [Transact APIs](ref:transact-api-quickstart) (except [`eth_sendPrivateTransaction`](ref:eth-sendprivatetransaction) and [`eth_cancelPrivateTransaction`](ref:eth-cancelprivatetransaction))
- [Transaction Receipts API](ref:transaction-receipts-endpoints) 
- [Token APIs](ref:token-api-quickstart) 
- [Subscription APIs](ref:subscription-api) (except [`newPendingTransactions`](ref:newpendingtransactions) [`newHeads`](ref:newheads) and [`logs`](ref:logs))
- [Trace APIs](ref:trace-api-quickstart) 
- [Debug APIs](ref:debug-api-quickstart) 

## How do you make a batch request?

Batch requests are formatted the same as individual API requests however, the body of the request is an array containing the individual API calls you wish to make, rather than the single API call. Check out the example with [eth_blockNumber](ref:eth-blocknumber) below:
...
Batch Requests are usually for JSON-RPC endpoints, but Alchemy SDK is providing support for batch requests over REST as well. It currently only supports one type of REST API for batch requests: [getNFTMetadataBatch](ref:getnftmetadatabatch). Check example code below.
...
Batch requests always return a `200` HTTP status code, even if some or all requests within the batch fail. Therefore, it is important to parse the [JSON-RPC error codes](ref:error-reference#standard-json-rpc-errors) in the response to determine if the individual requests within the batch succeeded or failed.