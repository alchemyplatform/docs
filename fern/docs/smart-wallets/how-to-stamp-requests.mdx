...
2. Generate a private key bundle 
   1. Exchange the `targetPublicKey` for an encrypted `bundle` by calling [create wallet](https://alchemyenterprisegroup.readme.io/reference/createaccount) (e.g., OAuth, email login)
...
4. Sign the JSON-encoded POST body with your private key (generated in step 3) to produce a `signature` _(DER-encoded)_
