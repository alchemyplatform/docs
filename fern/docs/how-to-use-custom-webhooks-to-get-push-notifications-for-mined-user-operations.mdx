...
subtitle: >-
  Learn how to stream real-time alerts for mined User Operation with Custom
  Webhooks + variables
slug: >-
  docs/how-to-use-custom-webhooks-to-get-push-notifications-for-mined-user-operations
---
...
<Info title = "UserOperationEvent">
  This will only work for entrypoints that emit the UserOperationEvent (defined by the EF [here](https://github.com/eth-infinitism/account-abstraction/blob/ver0.6.0/contracts/interfaces/IEntryPoint.sol)). The event signature for user operations is
</Info>
...
<Info title = "Non-empty variables + auth token">
...
<Warning title = "Match variable name + network">
...
<Warning title ="Address variable format">
  The addresses must be in byte32 hex format, so you will likely need to convert from the SCA sender address format used in the userOp signature. For example, you can use [web3.js padLeft](https://web3js.readthedocs.io/en/v1.2.11/web3-utils.html#padleft):
</Warning>
...
<Info title = "Only receive webhooks for non-empty data">
  If you would like to only receive webhooks for non-empty queries, rather than for every block, follow [these](/reference/custom-webhooks-faq#how-do-i-receive-webhooks-for-only-non-empty-results-ie-turn-off-the-heartbeat-mechanism-described-abovedont-receive-a-webhook-for-every-block) instructions to set the `skip_empty_messages`param to true in your query.
</Info>
...
<Warning>
  The user op hashes added to your variable must be in byte32 hex format, so you will likely need to convert. For example, you could use [web3.js padLeft](https://web3js.readthedocs.io/en/v1.2.11/web3-utils.html#padleft):
</Warning>
