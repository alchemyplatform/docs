---
title: 9. How to Build a Token Swap Dapp With 0x API
subtitle: >-
  ...
  ecosystem and surfaces the best price to the user.
slug: docs/how-to-build-a-token-swap-dapp-with-0x-api
---
...
![312](/images/cceeaeb-Matcha.png "Matcha.png")

Trading WETH\<>DAI using Matcha.xyz, a DEX aggregator.
...
The first thing you'll need to do is to clone the project starter code. We will be building on this starter UI to add token trading functionality.
...
Now, when you serve yourdapp, and inspect it, you should see the token list json object printed out. If this is your first time using Chrome's Inspect DevTool, checkout [this article](https://www.hostinger.com/tutorials/website/how-to-inspect-and-change-style-using-google-chrome) to learn more about it.
...
One functionality that's common in DeFi trading dapps that's really intuitive is automatically generating a price when the amount is inputted and the user's cursor leaves the focus on the amount box. Check it out here in [Matcha.xyz](https://matcha.xyz/).
...
<Info>
  This API endpoint is using the Ethereum (mainnet) as indicated by `https://api.0x.org` `\ `Here is a list of[ all API endpoints for the supported networks.](https://docs.0x.org/0x-api-swap/api-references#introduction)
</Info>
...
We will use the [BigNumber](https://github.com/MikeMcl/bignumber.js/) library to create a really large number. Then we set this to a `maxApproval` variable. Plug this in at the bottom of `trySwap()` in `index.js`
...
The reason we can directly pass in the [`/quote` response](https://docs.0x.org/0x-api-swap/api-references/get-swap-v1-quote#response) is because it contains all the necessary [parameters for web3.eth.setTransaction()](https://web3js.readthedocs.io/en/v1.2.11/web3-eth.html#id84) - *from, to, value, gas, data, etc*.
...
* Currently we set[ the token allowance is set to the max amount](https://docs.alchemy.com/alchemy/road-to-web3/weekly-learning-challenges/9.-how-to-build-a-token-swap-dapp-with-0x-api#ii-set-the-approval-amount-to-maxapproval). Change this to be safer so the user only approves just the amount needed.
...
Once you're done with your challenge, tweet about it by tagging [@AlchemyLearn](https://twitter.com/AlchemyLearn), [@0xProject](https://twitter.com/0xProject), and the author [@hey\_its\_jlin](https://twitter.com/hey_its_jlin) on Twitter!
