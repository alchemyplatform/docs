...
<Accordion title="Answer">
  This error occurs when you try to send multiple transactions simultaneously from an EOA that has been delegated via [EIP-7702](/wallets/transactions/using-eip-7702). When self-relaying transactions (i.e. sending transactions directly from the EOA rather than through a bundler), a delegated EOA can only have **1 transaction in the mempool at a time**. This is an inherent limitation of [EIP-7702 transaction propagation rules](https://eips.ethereum.org/EIPS/eip-7702#transaction-propagation).

  **Recommended solution:** Use EIP-7702 with Alchemy's bundler via our [Wallet APIs](/wallets/transactions/send-transactions) instead of self-relaying. When transactions are submitted through the bundler, you get **unlimited parallel transactions** because the bundler manages nonce ordering and submission on your behalf.
</Accordion>
