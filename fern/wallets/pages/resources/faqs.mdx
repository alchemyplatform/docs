...
<Accordion title="Answer">
  This error occurs when you try to send multiple transactions simultaneously from an EOA that has been delegated via [EIP-7702](/wallets/transactions/using-eip-7702). When self-relaying transactions (i.e. sending transactions directly from the EOA rather than through a bundler), a delegated EOA can only have **1 transaction in the mempool at a time**. This is an inherent limitation of how EIP-7702 delegated accounts work with standard transaction submission.
