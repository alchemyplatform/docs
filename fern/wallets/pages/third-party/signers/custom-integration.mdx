...
<Note>
  Using `@account-kit/wallet-client` (v4)? The `WalletClientSigner` wrapper is no longer needed in v5 — pass your `WalletClient` directly as the signer. See the [migration guide](/wallets/resources/migration-v5) for details.
</Note>
...
export async function createSmartEmbeddedWalletClient({
  ...
  const account = toAccount({
    ...
    // Delegate standard signing methods to the wallet provider
    signMessage: ({ message }) =>
