...
When using Alchemy Wallet APIs:

- Wallet APIs detect whether a user must first delegate via EIP-7702
- If delegation is required, Wallet APIs prepare the correct authorization payload
- Your application prompts the user to sign when required
- Alchemy combines the delegation and transaction into a single onchain submission
...
<Accordion title="How Wallet APIs handle delegation and signing">
  ...
  Alchemy combines these signatures into a single UserOperation and submits it to the bundler.
...
<Accordion title="Delegation-only (smart account upgrade) flows">
  ...
  Alchemy handles combining delegation with the user operation automatically.
</Accordion>
