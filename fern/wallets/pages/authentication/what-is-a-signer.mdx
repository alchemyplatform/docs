...
1. [**`LocalAccount`**](https://viem.sh/docs/accounts/local) — A viem local account, such as one created with [`privateKeyToAccount`](https://viem.sh/docs/accounts/privateKeyToAccount) or [`toAccount`](https://viem.sh/docs/accounts/toAccount). Useful for private key signers, hardware wallets, or custom account implementations.
...
For [EIP-7702](/wallets/transactions/using-eip-7702) delegation, the signer must support `signAuthorization`. See the [third-party signer integration guide](/wallets/third-party/signers/custom-integration) for details on bringing your own signer.
