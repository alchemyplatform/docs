...
Within Smart Wallets, we provide a number of implementations for `SmartAccountSigner` and `SmartAccountAuthenticator`:
...
2. [`privateKeyToAccount`](https://viem.sh/docs/accounts/privateKeyToAccount) (from `viem/accounts`) - This is useful if you have a private key locally that you want to use as a signer. This is useful for testing and development purposes.
3. [Viem `WalletClient`](https://viem.sh/docs/clients/wallet) - A viem [wallet client](https://viem.sh/docs/clients/wallet) can be passed directly as a signer. Since a wallet client can wrap any [EIP-1193](https://eips.ethereum.org/EIPS/eip-1193) compliant provider, this is useful if you want to use EOA extensions or 3rd party signer service SDKs.
