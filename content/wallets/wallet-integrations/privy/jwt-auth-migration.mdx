...
See the [Privy JWT setup docs](https://docs.privy.io) for full details.

### 2b. Disable automatic wallet creation

During migration, you don't want Privy to auto-create new wallets — you'll be importing existing ones. If using a client SDK, set `createOnLogin: "off"`:
...
async function handleMigration(userId: string) {
  ...
  if (!user.signer_migrated) {
    ...
    // 5. Import into Privy using the useImportWallet hook
...
### 4c. Authenticate with Alchemy

Use the same JWT to authenticate with the Alchemy signer. JWTs are reusable, so the same token works for both Privy and Alchemy in the same session.
...
// Authenticate with JWT — same token you used for Privy
await alchemySigner.authenticate({ type: "jwt", token: jwt });
```

### 4d. Export private key from Alchemy

Once authenticated, export the user's private key material.

```tsx
const privateKey = await alchemySigner.exportPrivateKey(...);

// If you have an encryption key, you can export the private key in an encrypted manner:
// Necessary for the server-side flow
const privateKeyEncrypted = await alchemySigner.exportPrivateKeyEncrypted(...);
```

### 4e. Import into Privy (client-side)
...
Use this path if your backend controls authentication and you want to handle the entire migration flow server-side — no client SDK required.
...
### 5b. Build the server-side migration flow

On each user login, your server checks whether the user needs migration. If so, it exports the key from Alchemy and imports it into Privy using the Node SDK or REST API.

```tsx
async function handleUserLogin(userId: string, jwt: string) {
  const user = await getUserFromDB(userId);

  if (!user.signer_migrated) {
    // 1. Export private key from Alchemy (Follow the steps from 4c)

    // 2. Import into Privy via the Node SDK
...
* **Server-side path:** The private key passes through your server. The `@privy-io/node` SDK automatically encrypts key material with HPKE before transmitting to Privy's TEE — the plaintext key never leaves your server unencrypted over the wire.
