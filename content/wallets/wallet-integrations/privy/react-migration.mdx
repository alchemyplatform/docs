...
During the migration window, set `createOnLogin` to `"off"` so Privy doesn't create a fresh wallet before the migration SDK can import the existing one. Once all users have migrated, switch it back to `"users-without-wallets"`.
