...
You can use any chain here — imported from `viem/chains`. Use whichever chain matches your Alchemy app configuration.
...
The migration SDK re-uses the Alchemy Account Kit UI under the hood. If `@account-kit/react` or `@account-kit/infra` aren't already resolved in your project (for example because you removed `@account-kit/react` in Step 1a and never had `@account-kit/infra` as a direct dependency), install them alongside the migration SDK:

```bash
npm install @account-kit/react @account-kit/infra
```
