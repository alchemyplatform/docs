...
<Info>
  **Use a custom ERC-20 token.** The tokens listed in the dashboard are not the
  limit. You can configure a token through the Admin API. Testnet tokens require
  a compatible price-reference token; mainnet tokens can use either a price
  supported by the [Token Prices By Address API](https://www.alchemy.com/docs/data/prices-api/prices-api-endpoints/prices-api-endpoints/get-token-prices-by-address)
