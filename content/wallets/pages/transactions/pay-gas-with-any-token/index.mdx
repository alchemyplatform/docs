...
<Info>
  **Use a custom ERC-20 token.** The tokens listed in the dashboard are not the
  ...
  a compatible price-reference token; mainnet tokens can use either their own
  supported price or an optional price reference.
</Info>
...
price. Mainnet tokens can omit both fields when their own token price is
supported, or set both fields to use a reference token instead.
