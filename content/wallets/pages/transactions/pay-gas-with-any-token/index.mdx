...
<Info>
  **Use a custom ERC-20 token.** The tokens listed in the dashboard are not the
  limit. You can configure any ERC-20 token through the Admin API by providing
  a compatible price-reference token. The custom token and its price reference
  must use the same number of decimals.
</Info>
...
Set `priceReferenceNetwork` and `priceReferenceTokenAddress` for each custom
token. The price-reference token provides the conversion price and must have
the same number of decimals as the custom token.
