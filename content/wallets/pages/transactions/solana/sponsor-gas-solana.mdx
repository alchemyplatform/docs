...
When `prefundRent` is enabled, the service simulates the sponsored transaction to estimate any rent that may be needed for CPI-driven account creation. If needed, it adds a `SystemProgram.transfer` from the paymaster to `payerKey` before returning the sponsored transaction, so `payerKey` holds enough SOL to cover the CPI's account creation.
