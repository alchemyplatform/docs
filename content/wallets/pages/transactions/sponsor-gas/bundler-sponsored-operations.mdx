...
description: Alchemy's recommended, advanced path for gas sponsorship, with faster confirmations and automatic retries.
subtitle: Alchemy's recommended, advanced path for gas sponsorship. BSO covers gas for your users' transactions with up to 30% faster execution and automatic retries built in.
slug: wallets/transactions/sponsor-gas/bundler-sponsored-operations
---

## What is Bundler Sponsorship?

Bundler Sponsored Operations (BSO) are Alchemy's advanced way to cover gas fees for your users' transactions. It's the **recommended default** for new gas sponsorship integrations: BSO lands transactions faster and retries them automatically.

### Advanced sponsorship features

* **Faster execution** — up to 30% faster inclusion (p99) vs. standard sponsorship. *Improvement varies by chain and network conditions.*
* **Automatic retries** — delayed or failed transactions are retried automatically, with no extra integration work. Just tell us how much you're willing to spend per transaction.
* **No onchain contracts** — nothing to deploy, fund, or manage onchain.
...
Create a Gas Manager policy in your [dashboard](https://dashboard.alchemy.com/). Configure:

* **Policy Type**: select **Bundler Sponsored Operations**
* **Max Spend Per UO**: the maximum spend per transaction (required)
...
1. **Create a policy** in your dashboard, selecting **Bundler Sponsored Operations** as the Policy Type and setting a **Max Spend Per UO**.
2. **Include the policy ID** in your requests when sending transactions.

### With Wallet APIs (recommended)

Pass your BSO policy ID as the `paymaster` option on the smart wallet client. Every call routed through the client is sponsored under that policy, and the SDK takes care of the rest.
...
If you're using viem's account-abstraction primitives directly, two extra steps are required:
