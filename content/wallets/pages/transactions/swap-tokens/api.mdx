...
Use your signer address directly as the `from` field to enable [EIP-7702](/docs/wallets/transactions/using-eip-7702) by default. Note that `postCalls` are optional and allow you to batch an array of calls after the swap.
...
If the account hasn't been delegated yet, the response type will be `array` containing both an EIP-7702 authorization and a user operation. See [Send Transactions](/docs/wallets/transactions/send-transactions) for handling authorization signing.
