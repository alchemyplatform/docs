...
<Accordion title="Preparing the EIP-7702 authorization yourself">
  ...
  const prepared = await smartWalletClient.prepareCalls({
    ...
    chainId: "0x14a34",
    capabilities: {
      paymasterService: { policyId: "YOUR_BSO_POLICY_ID" },
      eip7702Auth: {
  ...
  const { r, s, yParity } = await account.signAuthorization({
    contractAddress: prepared.data.address,
    chainId: parseInt(prepared.chainId, 16),
    nonce: parseInt(prepared.data.nonce, 16),
  });
