...
| [alchemy_requestPaymasterAndData](/docs/reference/alchemy-requestpaymasteranddata)                                                                                  | **\[To be deprecated]** Requests gas sponsorship for a `UserOperation` on EVM networks. If approved, returns`paymaster` and `paymasterData`.                                                                            |
...
| [pm_getPaymasterData](/docs/reference/pm_getpaymasterdata)                                                                                                          | Returns values to be used in paymaster-related fields of a signed user operation on EVM networks. These are not stub values and will be included in a signed user operation as part of an `eth_sendUserOperation` call. |
| [alchemy_requestFeePayer](/docs/reference/alchemy-requestfeepayer)                                                                                                  | Request gas sponsorship for a `transaction` on Solana. Returns the `serializedTransaction`, including the `feePayer` signature.                                                                                         |
