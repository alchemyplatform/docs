...
To allow the paymaster contract to transfer ERC-20 tokens from the user’s wallet, the user must first approve (`approve()`) the paymaster contract to spend tokens on their behalf. This typically requires a separate transaction, either paid for by the user with the native gas token or sponsored by the app builder.
...
1. \[Recommended] Transfer ERC-20 tokens *after* transaction execution:
   1. The gas is fronted using the native gas token and ERC-20 tokens are transferred from the user’s wallet to your receiving address after the transaction execution.
