...
A **signer** (also called an "owner") is the service or key that controls a smart wallet — it manages a private key, authenticates users, and signs transactions on their behalf. Your choice of signer determines the onboarding experience, custody model, and recovery options available to your users.

This guide covers the different signer types, the criteria to evaluate when choosing one, and how they plug into Wallet APIs.

## Role of a signer

The signer plays a crucial role in your app because it controls the user's smart wallet. The signer is responsible for:
...
For Solana transactions, use a `SolanaSigner` or one of the [Solana signer adapters](/docs/wallets/third-party/signers/solana-signers) from `@alchemy/wallet-apis/solana`.
