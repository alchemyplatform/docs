...
<Info>
  If you use Wallet APIs, follow the [pay gas with any token
  guide](/docs/wallets/transactions/pay-gas-with-any-token) instead. Wallet APIs
  can quote the payment, add the approval, prepare the operation, and return the
  signature request for you.
</Info>
...
<Steps>
  <Step title="Configure the RPC client and account hooks">
    ...
    type PartialUserOperationV07 = Pick<
      UserOperationV07,
      "sender" | "nonce" | "callData"
    > &
      Partial<Pick<UserOperationV07, "factory" | "factoryData">>;
    type UnsignedUserOperationV07 = Omit<UserOperationV07, "signature">;
  ...
  <Step title="Discover the paymaster address">
    An ERC-20 approval names a spender, so discover the paymaster before encoding the approval. Do not maintain a hardcoded paymaster address.
  ...
  <Step title="Quote the operation with its approval cost">
    ...
    const quoteOperation = await buildPartialUserOperation([
      approveErc20(paymentToken, paymaster, maxUint256),
      applicationCall,
    ]);
  ...
  <Step title="Build the final calls and request sponsorship">
    ...
    const finalCalls: Call[] = [
      ...(currentAllowance >= approvalAmount
        ? []
        : [approveErc20(paymentToken, paymaster, approvalAmount)]),
      applicationCall,
    ];
    ...
    Some tokens require an `approve(0)` call before changing a non-zero allowance. Account for that token-specific behavior when you build `finalCalls`.
...
```ts title="pay-gas-with-usdc.ts"
function approveErc20(token: Address, spender: Address, amount: bigint): Call {
  return {
    to: token,
    value: 0n,
...
function decimalTokenAmountToRawUnitsCeil(
  value: string,
  decimals: number,
): bigint {
  if (!/^\d+(?:\.\d+)?$/.test(value)) {
