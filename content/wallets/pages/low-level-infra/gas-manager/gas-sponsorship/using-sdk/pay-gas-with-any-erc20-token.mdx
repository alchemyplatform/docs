...
The application call only reads the account's USDC balance; running the example still spends USDC on gas. Replace that read with the call your application needs, and choose allowance and cap values appropriate for your application.
...
For reset-only tokens, include `approve(0)` before changing an existing non-zero allowance. The Base Sepolia USDC used here does not require that reset.
