...
Session keys are a powerful feature of the Wallets API that allow you to create a session for a user's smart account with specific permissions. This enables secure, permissioned access to the user's smart account, allowing your app's server to perform actions on behalf of the user without needing their private key. Session keys allow another account to operate on a user's smart account with given permissions. After creating a session, you can sign transactions for the smart account within the defined permissions using that session key. See [here for a list of permissions!](#permission-types)
