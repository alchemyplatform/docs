...
The following errors surface through Wallet APIs with error code `-32000`. The error message is prefixed with `An error occurred while executing user operation:` followed by the specific precheck failure.
...
<Info>
  ...
  * If needed, you can use [fee overrides](/docs/wallets/reference/alchemy-sendpreparedcalls#fee-overrides) to set a higher `maxFeePerGas` buffer, but this is an advanced override and shouldn't typically be required.
</Info>
...
{
  "code": -32000,
  "message": "precheck failed: EIP-7702 is not supported ...",
  ...
}
```

<Info>
  This chain does not support EIP-7702. You can use non-7702 mode by creating your account with [`wallet_requestAccount`](/docs/wallets/transactions/using-eip-7702). Reach out to Alchemy support if you need to inquire about EIP-7702 support on a specific chain.
</Info>
