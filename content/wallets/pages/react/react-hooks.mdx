...
The `useAccount` hook retrieves the wallet instance for the authenticated user. It's primarily used to get the wallet address and interact with the account.
...
* Returns a wallet instance (`account`) when the user is logged in
* Provides the wallet address, not the owner address
