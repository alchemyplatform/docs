...
The `useUser` hook returns the current user information from either an Externally Owned Account (EOA) or from a smart account. This is the best way to check if a user is logged in regardless of account type.
...
The `useAccount` hook retrieves the smart account instance for the authenticated user. It's primarily used to get the smart account address and interact with the account.
...
* Returns a smart account instance (`account`) when the user is logged in
* Provides the smart account address, not the owner address
