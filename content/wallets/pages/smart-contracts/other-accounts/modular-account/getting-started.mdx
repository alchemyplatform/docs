...
Next, if you want to use a different authentication provider with a smart account, check out [choosing an authentication provider](/docs/wallets/signer/what-is-a-signer). Otherwise, if you are ready to get onchain, go to [send transactions](/docs/wallets/transactions/send/send-user-operations).
...
To learn how to add and remove plugins (for example, installing the Session Key Plugin), see the guide on [Installing & uninstalling plugins](/docs/wallets/transactions/send/batch-user-operations).
