...
description: Learn about the ERC-6900 compatible Session Key Plugin.
slug: wallets/smart-contracts/other-accounts/modular-account/session-keys
---

The Session Key plugin lets your smart wallet add additional signers to your Modular Account with specific permissions.
...
Alternatively, you can require that a session key uses a specific paymaster contract address, instead of spending the account’s native token for gas.

Note that the gas limit is tracked in terms of native token units (wei), not in units of gas. The gas usage of a transaction is considered to be the maximum gas a transaction can spend, i.e. `total gas limit * maxFeePerGas`. This can overestimate when compared to the actual gas cost of each transaction.
