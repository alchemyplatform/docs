...
The Bundler is introduced by [ERC-4337](https://eips.ethereum.org/EIPS/eip-4337). It forwards transactions to the EntryPoint which are then further forwarded to the smart accounts for execution. The custom-built Bundler called Rundler provides high performance and reliability, written in Rust and [completely open source](https://github.com/alchemyplatform/rundler).

For deeper insights into this topic, explore the [blog post on smart wallets](https://www.alchemy.com//blog/account-abstraction).
...
<Info>
  The [Transaction APIs](/docs/wallets/transactions/overview) are the easiest
...
<Tip title="Skip fee estimation with Bundler Sponsored Operations">
  With [Bundler Sponsored Operations](/docs/reference/bundler-sponsored-operations), you can eliminate fee estimation entirely. Set a max spend limit per operation, pass zero for gas fees, and the bundler handles fee calculation, gas pricing, and automatic retries. No more computing `maxFeePerGas`, applying priority fee buffers, or implementing drop-and-replace logic.
</Tip>
