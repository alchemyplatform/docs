...
1. **Re-estimate gas fees**: This can be done in various ways which are mentioned below:
   ...
   2. If using the Alchemy SDK, use the [`getFeeData`](/docs/chains/ethereum/ethereum-api-endpoints/eth-max-priority-fee-per-gas) method to obtain both the current `maxPriorityFeePerGas` and `maxFeePerGas`. This method is also available on web3 libraries like `ethers.js` and can be accessed through the provider as `provider.getFeeData()`.
