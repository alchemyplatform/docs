...
1. **Re-estimate gas fees**: This can be done in various ways which are mentioned below:
   ...
   1. Use the [`eth_maxPriorityFeePerGas`](/docs/reference/eth-maxpriorityfeepergas) method to obtain `maxPriorityFeePerGas`.
   2. If using the Alchemy SDK, use the [`getFeeData`](/docs/reference/sdk-getfeedata) method to obtain both the current `maxPriorityFeePerGas` and `maxFeePerGas`. This method is also available on web3 libraries like `ethers.js` and can be accessed through the provider as `provider.getFeeData()`.
