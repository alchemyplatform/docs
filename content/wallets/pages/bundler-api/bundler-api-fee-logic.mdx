...
### How many compute units will it cost to create a wallet?

Deploy a wallet in two ways:

* **Contract deployment by sending a transaction**: Deploy the wallet by sending a transaction through [`eth_sendRawTransaction`](/docs/reference/eth-sendrawtransaction) from an EOA. This is same as other contract deployments. In this case, it costs 250 CUs which is the cost of calling `eth_sendRawTransaction`.
* **Contract deployment by sending a transaction via the bundler**: Deploy the wallet by sending a transaction through [`eth_sendUserOperation`](/docs/wallets/api-reference/bundler-api/bundler-api-endpoints/eth-send-user-operation) with a non-empty `initCode`. In this case it costs 1000 CUs which is the cost of calling `eth_sendUserOperation`.
