...
<Tip title="Want to skip fee estimation entirely?">
  [Bundler Sponsored Operations](/docs/reference/bundler-sponsored-operations) eliminate the need for manual fee estimation. Set a max spend limit, pass zero for gas fees, and the bundler handles fee calculation and automatic retries. The fee logic below only applies when you're managing fees yourself without bundler sponsorship.
</Tip>
...
### Compute units for smart wallet creation

Deploy a smart wallet in two ways:

* **Contract deployment by sending a transaction**: Deploy the smart wallet by sending a transaction through [`eth_sendRawTransaction`](/docs/reference/eth-sendrawtransaction) from an EOA. This is same as other contract deployments. In this case, it costs 250 CUs which is the cost of calling `eth_sendRawTransaction`.
* **Contract deployment by sending a transaction via the bundler**: Deploy the smart wallet by sending a transaction through [`eth_sendUserOperation`](/docs/wallets/api-reference/bundler-api/bundler-api-endpoints/eth-send-user-operation) with a non-empty `initCode`. In this case it costs 1000 CUs which is the cost of calling `eth_sendUserOperation`.
