...
All chains now come from `viem/chains`, all signers use viem-native types (`LocalAccount` or `WalletClient`). Interfaces are now fully viem-compatible — values are bigints instead of hex strings (`BigInt(0)` instead of `"0x00"`).

## EIP-7702 by default

The biggest change: the client defaults to EIP-7702. For 7702 accounts, you no longer need to call `requestAccount()` before sending transactions.
