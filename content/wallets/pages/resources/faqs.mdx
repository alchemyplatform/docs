...
<Accordion title="Answer">
  In almost all cases, yes, you get the same address on all chains as long as the connecting owner address is the same. The deployment address is a function of the owner address, the account implementation (e.g. latest version of Light Account), and the salt (you can optionally specify this). If all three remain the same, the smart account deploys at the same contract address.

  There are two scenarios where you would get a different contract address:

  1. If you deploy one smart account, then change the owner, then deploy the second account.
  2. If you upgrade the smart account (e.g. to a new version of Light Account). Updates to this contract are infrequent, so the address will not change often.
</Accordion>

### How does a smart account get deployed?

<Accordion title="Answer">
  Your smart account deploys when the first `UserOperation` (UO) is
...
### Can I have multiple accounts for the same owner address? / How do I set the value of the salt for Light Account?
...
### Why am I seeing a delay in landing `UserOperation`s onchain?
...
### How does gas estimation for 4337 smart accounts work?
...
<Accordion title="Answer">
  ...
  ***
  ...
  Use this flow to verify user sessions created by EOAs and Smart Contract Accounts (SCAs) via Smart Wallets.
