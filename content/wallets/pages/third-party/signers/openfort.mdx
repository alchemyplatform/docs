...
<Tabs>
  <Tab title="React" language="react">
    ...
    Follow these steps to use Openfort signers with the Wallet Client SDK using the `@openfort/react` SDK.
    ...
    * The `@openfort/react` SDK integrates with [wagmi](https://wagmi.sh/), so the signer is obtained via wagmi's `useWalletClient` hook. Since Openfort provides a `WalletClient` (not a `LocalAccount`), for EIP-7702 delegation you may need to use [non-7702 mode](/wallets/transactions/using-eip-7702#how-to-use-non-7702-mode). See the [EIP-7702 guide](/wallets/transactions/using-eip-7702) for details.
