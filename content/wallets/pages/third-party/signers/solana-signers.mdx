...
* Privy Solana peer dependencies: `@solana/kit @solana-program/memo @solana-program/system @solana-program/token`
...
| --- | --- |
| Privy Solana wallet from `@privy-io/react-auth/solana` | No adapter needed |
...
Use `useWallets` from `@privy-io/react-auth/solana` to get connected Solana wallets. The returned wallet can be passed directly to `createSmartWalletClient`.

```tsx title="privy-solana-signer.tsx"
import {
