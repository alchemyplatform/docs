...
<Warning>
  ...
  `@account-kit/core` and `@account-kit/react` are the only packages that are opinionated about which authentication provider you can attach to a smart account. However, if you use `@aa-sdk/*` or `@account-kit/infra` with `@account-kit/smart-contracts`, you can
