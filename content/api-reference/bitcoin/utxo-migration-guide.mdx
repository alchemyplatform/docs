...
UTXO API reference: [Bitcoin](/docs/chains/bitcoin/utxo-api-endpoints/utxo-api-endpoints/get-block-hash-by-index), [Bitcoin Cash](/docs/chains/bitcoincash/utxo-api-endpoints/utxo-api-endpoints/get-block-hash-by-index), [Litecoin](/docs/chains/litecoin/utxo-api-endpoints/utxo-api-endpoints/get-block-hash-by-index), [Dogecoin](/docs/chains/dogecoin/utxo-api-endpoints/utxo-api-endpoints/get-block-hash-by-index).
...
| Fiat rates in response   | Included via `fiatcurrency` param    | Not included (separate endpoint)       |
...
We maintain a live dashboard with real-time performance metrics for our UTXO endpoints across all supported networks, benchmarked against QuickNode and Blockdaemon. [View live dashboard](https://alchemyinsights.grafana.net/d/bbv2-compare/2b5a8ae?orgId=1&from=now-15m&to=now&timezone=browser&var-network=BITCOIN_TESTNET4&var-providers=$__all&var-region=$__all&refresh=5m).
...
<AccordionGroup>
  <Accordion title="Why should I migrate my UTXO calls to Alchemy?">
  ...
  <Accordion title="Is Alchemy's UTXO API a drop-in replacement?">
  ...
  <Accordion title="What about fiat price data in balance history responses?">
    QuickNode includes a `rates` object in balance history responses when the `fiatcurrency` parameter is set. Alchemy's balance history endpoint focuses on onchain data. Fiat pricing is available through a separate endpoint.
  </Accordion>
  <Accordion title="How do I enable UTXO endpoints on my Alchemy account?">
  ...
  <Accordion title="I'm migrating from BlockCypher. How different is the integration?">
  ...
  <Accordion title="I'm migrating from Blockdaemon. Does Alchemy support the same data?">
  ...
  <Accordion title="Where can I get help with my migration?">
