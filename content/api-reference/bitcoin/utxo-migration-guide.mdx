...
- **What about fiat price data in balance history responses?**
  - QuickNode includes a `rates` object in balance history responses when the `fiatcurrency` parameter is set. Alchemy's balance history endpoint focuses on on-chain data. Fiat pricing is available through a separate endpoint.
