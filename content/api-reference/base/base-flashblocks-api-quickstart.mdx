---
title: Base Flashblocks API quickstart
...
Flashblocks-enabled WebSocket subscriptions are also supported via `eth_subscribe`.

Use your Base WebSocket endpoint:

- `wss://base-mainnet.g.alchemy.com/v2/<YOUR_API_KEY>`
- `wss://base-sepolia.g.alchemy.com/v2/<YOUR_API_KEY>`

To create a subscription, open a WebSocket connection and send an `eth_subscribe` request.
...
After subscribing, events are delivered through `eth_subscription` notifications.
...
For the full list of Flashblocks subscription types and payloads, see [Flashblocks subscription types](https://docs.base.org/base-chain/flashblocks/api-reference#flashblocks-subscription-types).

For more details, see the [Official Base Flashblocks Docs](https://docs.base.org/base-chain/flashblocks/apps).
