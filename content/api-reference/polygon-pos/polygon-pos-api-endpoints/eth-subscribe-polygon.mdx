---
title: eth_subscribe - Polygon PoS
description: Subscribe to different Polygon event types like newHeads, logs, pendingTransactions, and minedTransactions using WebSockets.
subtitle: Subscribe to different Polygon event types like newHeads, logs, pendingTransactions, and minedTransactions using WebSockets.
slug: reference/eth-subscribe-polygon
---

# eth\_subscribe - Polygon PoS
...
<CodeGroup>
  ...
  const client = createPublicClient({
    chain: polygon,
    transport: webSocket('wss://polygon-mainnet.g.alchemy.com/v2/demo') // Replace with your API key
  })

  // Subscribe to pending transactions with manual filtering
  const unsubscribe = client.watchPendingTransactions({
    onTransactions: async (hashes) => {
      for (const hash of hashes) {
        const tx = await client.getTransaction({ hash })
        // Manual filtering for to/from addresses since Viem doesn't have built-in filters
        if ((tx.to && tx.to.toLowerCase().includes('vitalik')) ||
            (tx.from && tx.from.toLowerCase().includes('shah'))) {
          console.log(tx)
        }
      }
    }
