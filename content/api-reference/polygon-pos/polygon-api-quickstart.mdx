...
<Warning title="Erigon client deprecation on Polygon PoS">
  Polygon Labs is sunsetting the Erigon client on Polygon PoS. Alchemy is migrating Polygon Mainnet and Amoy infrastructure from Erigon to **Bor** on **August 1, 2026**. After this date, the `trace_*` and `erigon_*` namespaces will no longer be available on Polygon Mainnet (`polygon-mainnet`) or Polygon Amoy (`polygon-amoy`). See the [Polygon PoS Erigon Client Deprecation Notice](/docs/reference/polygon-pos-erigon-deprecation-notice) for the full migration guide.
</Warning>
