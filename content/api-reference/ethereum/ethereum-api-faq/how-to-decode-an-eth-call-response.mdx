...
Learn how to decode the overwhelmingly large hex strings returned by the [`eth_call`](/docs/reference/eth-call) method.

# Introduction

The [`eth_call`](/docs/reference/eth-call) method is commonly used to call the read-only functions of smart contracts, like reading the owner off an NFT smart contract. The values returned by these functions are in the hex format. To convert these values to a human-readable format, we need to decode them. In this guide, we will show you how to decode the responses of the `eth_call` method so that you can easily read them.

# Prerequisites

To continue with this guide you need to about the [eth\_call](/docs/reference/eth-call) method and how it works.

# Making an `eth_call` request

For the sake of this tutorial, we will be using the `eth_call` method on an ERC20 token contract (USDT) to get the balance of a particular address and also get the symbol of the token. We will be calling the [eth\_call](/docs/reference/eth-call) method using Ethers.js, which provides excellent built-in support for ABI encoding/decoding.

<Info>
  It would be way easier to get the the token balance of an address and the symbol of the token using Alchemy's [`getTokenBalances`](/docs/reference/alchemy-gettokenbalances) and [`getTokenMetadata`](/docs/reference/alchemy-gettokenmetadata) APIs as you won't have to deal with decoding the response in that case.
</Info>
...
Next, navigate to the [Alchemy Dashboard](https://dashboard.alchemy.com/signup) and [create a new app](/docs/alchemy-quickstart-guide). Make sure you set the chain to Ethereum and the network to Mainnet. Once the app is created, click on your app's *View Key* button on the dashboard. Take note of the **HTTP URL**.
...
In order to get the balance of a token for a wallet address and its symbol, we need to leverage the [eth\_call](/docs/reference/eth-call) method. This will allow us to query USDT contract's `balanceOf` method and the `symbol` state variable.

Add the following code to the `main.js` file. Here's an overview of what the code is doing (also explained in comments):

* Imports the required objects from Ethers.js.
* Creates a provider connected to Alchemy's endpoint.
...
* Creates function call data for getting the symbol and balance using Ethers.js Interface.
* Makes an `eth_call` request using Ethers.js for getting the symbol value in the hex format.
* Makes an `eth_call` request using Ethers.js for getting the balance value in the hex format.
...
<CodeGroup>
  ...
  const main = async () => {
    ...
    const abi = ["function balanceOf(address account)", "function symbol()"];
    ...
    const balanceData = iface.encodeFunctionData("balanceOf", [walletAddress]);

    // Get symbol of the token in hex format -- usage of eth_call
    const symbolInHex = await provider.call({
      to: contractAddress,
      data: symbolData,
    });
...
* Defines the number of decimals of the ERC20 token.
* Defines the abi for the ERC20 token that will be used for making an `eth_call` request.
* Creates function call data for getting the symbol and balance using the ABI and Ethers.js Interface.
* Makes an `eth_call` request using Ethers.js for getting the symbol value in the hex format.
* Makes an `eth_call` request using Ethers.js for getting the balance value in the hex format.
