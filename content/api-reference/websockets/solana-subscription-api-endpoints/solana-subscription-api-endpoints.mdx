...
Solana WebSocket subscriptions on Alchemy are billed by **bandwidth**: **$80 per 1 TB** streamed, pro-rated by bytes delivered. [Contact us](https://www.alchemy.com/contact-sales) for pre-committed bulk discounts.

<Warning>
  Broad streams can produce very high bandwidth, especially `programSubscribe` without filters and `blockSubscribe` with `transactionDetails: "full"`. To keep usage predictable:
