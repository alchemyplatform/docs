...
<CodeGroup>
  ...
  ```text Devnet
  wss://solana-devnet.streaming.alchemy.com/v2/<-- ALCHEMY APP API KEY -->
  ```
</CodeGroup>

<Info>
  Solana PubSub WebSocket subscriptions are supported on both Solana Mainnet and Solana Devnet, and pricing is the same on both networks. To switch networks, swap `solana-mainnet` for `solana-devnet` in the URL.
</Info>
