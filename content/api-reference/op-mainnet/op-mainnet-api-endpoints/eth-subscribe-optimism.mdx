---
title: eth_subscribe
description: Subscribe to different Optimism event types like newHeads, logs, and syncing using WebSockets.
subtitle: Subscribe to different Optimism event types like newHeads, logs, and syncing using WebSockets.
slug: reference/eth-subscribe-optimism
---
...
* Event Object:
  ...
  * `params`: An object with the following fields:
    ...
    * `subscription`: The subscription ID returned by the `eth_subscribe` call which created this subscription. This ID will be attached to all received events and can also be used to cancel the subscription using [`eth_unsubscribe`](/docs/reference/eth-unsubscribe)
...
<CodeGroup>
  ...
  const client = createPublicClient({
    chain: optimism,
    transport: webSocket('wss://opt-mainnet.g.alchemy.com/v2/demo') // Replace with your API key
  })

  // Subscribe to new blocks on Optimism
  ...
  // For Alchemy's pendingTransactions with address filtering, use raw WebSocket
  const ws = new WebSocket('wss://opt-mainnet.g.alchemy.com/v2/demo')

  ws.onopen = () => {
    // Subscribe to alchemy_pendingTransactions with filtering
  ...
  ws.onmessage = (event) => {
    ...
    if (data.method === 'eth_subscription') {
      console.log('Filtered pending transaction:', data.params.result)
    }
  }
  ```
  ...
  const provider = new ethers.WebSocketProvider('wss://opt-mainnet.g.alchemy.com/v2/demo')
  ...
  // For Alchemy's pendingTransactions with address filtering
  provider.send('eth_subscribe', [
    ...
    provider.on(subscriptionId, (transaction) => {
      console.log('Filtered pending transaction:', transaction)
    })
  })
  ```
</CodeGroup>
