...
<Warning title="Deprecation notice">
  **Action required:** Support for Sui JSON-RPC will end on **September 25, 2026**. Please migrate to **Sui gRPC** before then to avoid disruption. You can find our migration guide [here](/docs/reference/sui-json-rpc-migration-guide).
</Warning>
...
| `sui_subscribeTransaction`                                            | `SubscriptionService.SubscribeTransactions`         | Same filter type as `ListTransactions`, so backfill and live stream share one filter                        |
...
| `sui_subscribeEvent`                 | `SubscriptionService.SubscribeEvents`        | Same `EventFilter` type as `ListEvents`                                            |
...
`sui_subscribeEvent` and `sui_subscribeTransaction` are replaced by server-streaming RPCs on `SubscriptionService`. Three differences matter in production:
...
Replaces `sui_subscribeEvent`. Pair it with `LedgerService/ListEvents` using the same filter to backfill.
