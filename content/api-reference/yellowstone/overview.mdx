...
<Info>
  Yellowstone gRPC is supported on both Solana Mainnet (`https://solana-mainnet.streaming.alchemy.com`) and Solana Devnet (`https://solana-devnet.streaming.alchemy.com`). Both endpoints use the same `X-Token` authentication and the same `SubscribeRequest` API. Pricing is the same on both networks.
</Info>
