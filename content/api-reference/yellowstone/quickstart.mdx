...
<Info>
  Yellowstone gRPC is supported on both Solana Mainnet and Solana Devnet. The examples below use the Mainnet endpoint (`https://solana-mainnet.streaming.alchemy.com`); to target Devnet, swap `solana-mainnet` for `solana-devnet` in the URL. Both endpoints use the same `X-Token` auth header and the same pricing.
</Info>
