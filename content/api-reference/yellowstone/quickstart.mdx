...
<Info>
  Yellowstone gRPC is supported on both Solana Mainnet and Solana Devnet. The examples below use the Mainnet endpoint (`https://solana-mainnet.streaming.alchemy.com`); to target Devnet, replace it with `https://solana-devnet.g.alchemy.com` (note the different host base). Both endpoints use the same `X-Token` auth header and the same pricing.
</Info>
...
```rust
let endpoint = "https://solana-mainnet.streaming.alchemy.com";
