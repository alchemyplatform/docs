---
title: DATA Network API Quickstart
description: How to get started building on DATA Network using Alchemy
subtitle: How to get started building on DATA Network using Alchemy
...
Let's use the [`viem`](https://www.npmjs.com/package/viem) package to create a DATA Network client connected to Alchemy and fetch the latest block number!
...
// DATA Network is currently exported as `story` in viem/chains (chain ID 1514).
...
console.log("Balance (DATA):", Number(balance) / 1e18);
```
</CodeGroup>
...
For the full list of DATA Network APIs, see the [DATA Network API Endpoints](/docs/chains#data-network-apis).
