...
* Limits are evaluated over a 10-second rolling window using a token bucket algorithm with strict refill rates. For example, a Free tier customer with a 300 CU/s limit can consume up to 3,000 CUs over any 10-second period. This design helps accommodate short-term traffic spikes without exceeding the overall limit.
