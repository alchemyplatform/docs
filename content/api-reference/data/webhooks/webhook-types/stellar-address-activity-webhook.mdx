...
<Info>
  See the [Feature Support by Chain](/docs/reference/feature-support-by-chain) page for details about product and chain support!
</Info>
...
<Warning>
  Stellar networks (`STELLAR_MAINNET` and `STELLAR_TESTNET`) are not yet part of the `network` enum accepted by the [Create Webhook](/docs/data/webhooks/webhooks-api-endpoints/notify-api-endpoints/create-webhook) endpoint. Until the Notify API adds them, create Stellar Address Activity webhooks through the [Alchemy Dashboard](https://dashboard.alchemy.com/apps/latest/webhooks) using the setup steps above. Once created, the address-management, update, and delete endpoints below operate on the webhook by ID and work as expected.
</Warning>
