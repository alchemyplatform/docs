...
Stream replay covers a short reconnect window only; it is not a historical query mechanism. For block-derived streams, gaps wider than that window are recovered with the block lookup methods. Indexed historical APIs and bulk export are separate surfaces, and coverage varies by dataset.
