...
* **Streaming support**: Subscribe to real-time checkpoint, transaction, and event streams with server-streaming RPCs
...
| `ListCheckpoints` | `sui_getCheckpoints` |
| `ListTransactions` | `suix_queryTransactionBlocks` |
| `ListEvents` | `suix_queryEvents` |
...
Stream real-time checkpoints, transactions, and events through the `SubscriptionService`.

| gRPC method | JSON-RPC equivalent |
| --- | --- |
| `SubscribeCheckpoints` | No equivalent |
| `SubscribeTransactions` | `suix_subscribeTransaction` |
| `SubscribeEvents` | `suix_subscribeEvent` |
