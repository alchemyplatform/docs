...
When `groupBy` includes `method`, each query can span at most 14 days. Enterprise teams can still start up to 90 days ago, but must split longer ranges into separate 14-day queries. The API returns one day per page for those queries.
