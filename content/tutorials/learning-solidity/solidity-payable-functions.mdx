...
Payable functions in Solidity are functions that let a smart contract accept Ether. They help you manage incoming Ether and take actions when it's received. For example, a simple payable function can collect Ether donations for a fundraiser. Here's a basic code example:
...
Sending ether is a native function of [the Ethereum Virtual Machine (EVM)](https://www.alchemy.com/overviews/what-is-the-ethereum-virtual-machine-evm). This is different from any other transfer in the EVM which requires custom logic inside a smart contract to handle the transfer (for example, NFTs or ERC-20s).
...
For example, if this was a payable smart contract controlled by a charity accepting cryptocurrency donations, callers would call **deposit** and the charity would eventually be able to withdraw these contributions to an address of their choosing. In that case, it may be better to write a **receive** function:
...
# Revertible payable function example
...
# Solidity payable functions summary

In summary, a **payable** function is a function that can receive ether. It provides you with the opportunity to respond to an ether deposit for record-keeping or any additional necessary logic.
