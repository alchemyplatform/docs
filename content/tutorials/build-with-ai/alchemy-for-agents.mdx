...
description: Authenticate your AI agent with a crypto wallet and purchase Alchemy API credits with USDC using the x402 protocol.
subtitle: Authenticate your AI agent with a crypto wallet and purchase Alchemy API credits with USDC using the x402 protocol.
slug: docs/alchemy-for-agents
---

Authenticate your AI agent with a crypto wallet instead of an API key. Your agent purchases credits with USDC via the [x402 payment protocol](https://www.x402.org/), with no human in the loop. We support authentication via SIWE (Sign-In With Ethereum) and SIWS (Sign-In With Solana), with payment via USDC on Base and Solana.
...
- **[x402](https://www.x402.org/):** An open protocol for HTTP-native payments. When your agent's credits run out, the server responds with HTTP 402, your agent purchases more credits with USDC, and retries the request. Zero protocol fees.
...
3. **Credits:** Your agent purchases credits with USDC from your wallet. When credits run out, the server responds with HTTP 402, and your agent purchases more automatically.
