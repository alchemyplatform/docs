...
| `alchemy evm approve <spender_address>` | Approve, revoke, or reset ERC-20 allowances |
...
| Command | Description |
|---|---|
| `alchemy wallet connect` | Connect a wallet. Use `--mode session` or `--mode local`. |
...
| `alchemy evm approve <spender_address>` | Approve, revoke, or reset ERC-20 allowances with exactly one of `--amount`, `--unlimited`, or `--revoke`. |
| `alchemy evm swap quote --from <addr> --to <addr> --amount <n>` | Quote a same-chain swap |
| `alchemy evm swap execute --from <addr> --to <addr> --amount <n>` | Execute a same-chain swap |
| `alchemy xchain bridge quote --from <addr> --to <addr> --amount <n> --to-network <id>` | Quote a cross-chain bridge. Source network comes from `-n, --network`. |
| `alchemy xchain bridge execute --from <addr> --to <addr> --amount <n> --to-network <id>` | Execute a cross-chain bridge |
...
| `alchemy solana status [id]` | Check a Solana transaction signature or operation ID |
...
| `alchemy app update <id> --name <name> --description <desc>` | Update app name or description |
| `alchemy app delete <id>` | Delete an app. Supports `--dry-run` and `-y, --yes`. |
| `alchemy app networks <id> --networks <ids>` | Update the network allowlist |
| `alchemy app address-allowlist <id> --addresses <addrs>` | Update the address allowlist |
| `alchemy app origin-allowlist <id> --origins <origins>` | Update the origin allowlist |
| `alchemy app ip-allowlist <id> --ips <ips>` | Update the IP allowlist |
...
| `alchemy config set <key> <value>` | Set config values such as `api-key`, `access-key`, `webhook-api-key`, `app`, `network`, `verbose`, `wallet-key-file`, `x402`, gas sponsorship, and fee sponsorship |
...
The `app` commands use a separate access key from the [Alchemy Dashboard](https://dashboard.alchemy.com/) → Access Keys.

```bash
alchemy config set access-key YOUR_ACCESS_KEY
```

You can also set `ALCHEMY_ACCESS_KEY` in your environment.
...
For autonomous workflows, the CLI can authenticate API requests with a wallet using the [x402 payment protocol](docs/alchemy-for-agents) instead of an API key. x402 uses a local EVM key for gateway authentication and is separate from the `alchemy wallet use` active signer used by EVM transaction commands.

```bash
alchemy config set wallet-key-file ./wallet.key
alchemy --x402 evm data balance vitalik.eth
```
