...
alchemy usage timeseries --start-date 2026-06-01 --group-by network
alchemy usage timeseries --start-date 2026-06-01 --group-by app,network
```

`timeseries` accepts a date or time range, optional filters, and up to three group-by dimensions.

Usage time-series lookback is limited by plan. Each request may span at most 30
days between `startTime` and `endTime`. Longer history requires multiple
queries. Cursors expire after 15 minutes. Cursor follow-up pages do not count
against the distinct-payload rate limit.
...
If you request a range that starts before your plan allows, the Usage API clamps
the start and omits older buckets. If the span exceeds 30 days, the API returns
HTTP 400. The CLI shows the requested and returned ranges when clamping happens.
...
| `--group-by <dimensions>` | Group by up to three dimensions: `requestType`, `app`, `network`, `method` (comma-separated) |

Filters are optional and combined with AND. `--group-by` accepts at most three unique dimensions.
