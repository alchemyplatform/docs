---
title: Alchemy CLI
description: Query blockchain data, call JSON-RPC methods, and manage your Alchemy account from the terminal. Built for live querying, admin work, and local automation by humans and AI agents alike.
subtitle: Query blockchain data, call JSON-RPC methods, and manage your Alchemy account from the terminal. Built for live querying, admin work, and local automation by humans and AI agents alike.
slug: docs/alchemy-cli
---

The Alchemy CLI is a single binary that wraps every Alchemy product, including JSON-RPC nodes, the Data APIs, Smart Wallets, the Notify webhook API, and the Admin API. Use it to:

* **Live query onchain data** without writing a script (`alchemy balance vitalik.eth`, `alchemy tx 0x...`)
* **Run admin tasks** like creating apps, rotating allowlists, and managing webhooks
* **Automate local workflows** in shell scripts, cron jobs, and CI pipelines
* **Give AI agents a structured tool surface** with `--json --no-interactive` output and a built-in `agent-prompt` command
...
To upgrade later, re-run the install command or check for updates:
...
The CLI uses different credentials depending on the command family. The fastest path is to pick the auth method that matches what you want to do.

| Auth method | Where to get it | Used by |
|---|---|---|
| **API key** | [Alchemy Dashboard](https://dashboard.alchemy.com/) → your app | `balance`, `tx`, `block`, `rpc`, `trace`, `debug`, `tokens`, `nfts`, `transfers`, `prices`, `portfolio`, `simulate`, `solana`, `bundler`, `gas-manager` |
| **Access key** | [Alchemy Dashboard](https://dashboard.alchemy.com/) → Access Keys | `apps` |
| **Webhook API key** | [Alchemy Dashboard](https://dashboard.alchemy.com/) → Notify | `webhooks` |
| **x402 wallet** | Generated locally with `alchemy wallet generate` | Most node and data commands, no API key required |

You can pass credentials per-command with flags (`--api-key`, `--access-key`, `--webhook-api-key`), set them as environment variables (`ALCHEMY_API_KEY`, `ALCHEMY_ACCESS_KEY`, `ALCHEMY_WEBHOOK_API_KEY`), or persist them with `alchemy config set`.

### Sign in once with your Alchemy account

Browser-based login pulls keys for you and configures the CLI in one shot:
...
alchemy config set network base-mainnet
alchemy config set app YOUR_APP_ID
```

Use `alchemy config list` to see saved values and `alchemy config reset <key>` to clear one.

### Use a wallet instead of an API key

For autonomous workflows, the CLI can authenticate with a wallet using the [x402 payment protocol](docs/alchemy-for-agents). Your agent or script signs requests with a local key and pays for credits in USDC.
...
After this, all node and data commands route through the gateway and bill from your wallet. See [Agent Authentication and Payment](docs/alchemy-for-agents) for the full flow.

## First commands

Once you have an API key set, try a few queries.
...
| Command | Description |
|---|---|
| `alchemy auth login` | Sign in via browser. Supports `--force` to re-authenticate. |
