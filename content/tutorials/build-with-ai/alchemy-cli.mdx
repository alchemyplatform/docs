...
For other auth methods (manual API key, webhook key for Notify, x402 wallet), see [Authentication options](#authentication-options) at the bottom of this page.
...
| `alchemy config set <key> <value>` | Set config values such as `api-key`, `webhook-api-key`, `app`, `network`, `verbose`, `wallet-key-file`, `x402`, gas sponsorship, and fee sponsorship |
