...
| | Remote server | Local (STDIO) |
|---|---|---|
| **Transport** | [Streamable HTTP](https://modelcontextprotocol.io/specification/2025-06-18/basic/transports#streamable-http) | [STDIO](https://modelcontextprotocol.io/specification/2025-06-18/basic/transports#stdio) |
| **Auth** | OAuth 2.1 (browser sign-in) | API key via env var |
| **Setup** | URL only — no install needed | `npx @alchemy/mcp-server` |
| **Best for** | Most users | CI / headless environments |

## Remote server (recommended)

The remote server runs at `https://mcp.alchemy.com/mcp` and authenticates via OAuth — just sign in with your Alchemy account when prompted. No API key or local install required.
...
If you prefer to run the server locally — or need it for CI, scripts, or headless environments — use the npm package with an API key.
...
Add to `~/.codeium/windsurf/mcp_config.json` (macOS) or `%USERPROFILE%\.codeium\windsurf\mcp_config.json` (Windows):
...
```bash
ALCHEMY_API_KEY=YOUR_API_KEY codex --mcp-server "npx -y @alchemy/mcp-server"
```
...
<Tip>
When using the remote server, steps 1-2 happen automatically after OAuth sign-in. With the local server, the API key from your environment is used directly.
</Tip>
