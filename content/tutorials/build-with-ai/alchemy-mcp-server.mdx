...
- `getBalance` — Get wallet balances across networks

**Transactions and transfers**
- `fetchMultichainWalletAddressTransactionHistory` — Get multi-chain transaction history
...
- `getNFTsForOwner` — Look up cross-network NFT ownership

**Advanced (requires wallet agent server)**
- `sendTransaction` — Send smart contract account transactions
- `swap` — Swap tokens via Uniswap

> The `sendTransaction` and `swap` tools require an `AGENT_WALLET_SERVER` environment variable pointing to a configured wallet agent server.
...
- [GitHub: alchemyplatform/alchemy-mcp-server](https://github.com/alchemyplatform/alchemy-mcp-server) — Source repository and full documentation
- [npm: @alchemy/mcp-server](https://www.npmjs.com/package/@alchemy/mcp-server) — npm package
