---
title: Build with AI
description: Give your AI agent knowledge of Alchemy APIs with agent skills, query live onchain data through MCP, and authenticate autonomously with x402 wallet auth.
subtitle: Give your AI agent knowledge of Alchemy APIs with agent skills, query live onchain data through MCP, and authenticate autonomously with x402 wallet auth.
...
Your agent can authenticate with a crypto wallet instead of an API key, paying per request with USDC via the [x402 payment protocol](https://www.x402.org/). This is what the `agentic-gateway` skill teaches your agent automatically.

- Supports EVM wallets ([SIWE](https://eips.ethereum.org/EIPS/eip-4361), USDC on Base) and Solana wallets (SIWS, USDC on Solana)
- No API key required
- Wallet type is independent of which chain you query

[Learn about x402 wallet authentication →](docs/alchemy-for-agents)
...
| Build an autonomous agent that authenticates with a wallet and pays with USDC | [Agent Skills](docs/alchemy-agent-skills) + [x402 wallet auth](docs/alchemy-for-agents) |
...
| Tool | Skills | MCP Server | x402 wallet auth |
|---|---|---|---|
